...
public class SpectrumTimeAnalyzer extends JPanel implements DigitalSignalProcessor {
    private static final long serialVersionUID = -5656961821269589305L;
    ...
    private DigitalSignalProcessingAudioDataConsumer dsp = null;
    ...
    public void startDSP(SourceDataLine line) {
        ...
        if (dsp == null) {
            dsp = new DigitalSignalProcessingAudioDataConsumer(2048, fps);
            dsp.add(this);
        }
        if ((dsp != null) && (this.line != null)) {
            if (dspStarted == true)
            {
                stopDSP();
            }
    ...
    public void setupDSP(SourceDataLine line) {
        if (dsp != null) {
            ...
            if (channels == 1) {
                dsp.setChannelMode(DigitalSignalProcessingAudioDataConsumer.CHANNEL_MODE_MONO);
            } else {
                dsp.setChannelMode(DigitalSignalProcessingAudioDataConsumer.CHANNEL_MODE_STEREO);
            }
            int bits = line.getFormat().getSampleSizeInBits();
            if (bits == 8) {
                dsp.setSampleType(DigitalSignalProcessingAudioDataConsumer.SAMPLE_TYPE_EIGHT_BIT);
            } else {
                dsp.setSampleType(DigitalSignalProcessingAudioDataConsumer.SAMPLE_TYPE_SIXTEEN_BIT);
            }
        }
    }
     ...
     * @return
     */
    public DigitalSignalProcessingAudioDataConsumer getDSP() {
        return dsp;
    }
    ...
    public void setVisColor(String viscolor) {
        ...
        try {
            ...
            List<Color> specList = Arrays.asList(specColors);
            ...
            logger.warn("Cannot parse viscolors", ex);
        }
        finally {
            try {
                if(bin != null) {
    ...
    public Color getColor(String linecolor) {
        ...
        try {
            ...
            logger.debug("Cannot parse viscolor : "+e.getMessage());
        }
        return color;
    }
